...
```bash
# result/visualization 폴더가 있으면 old/로 이동
...
# MPS (Mac) 또는 CPU 사용
uv run scripts/train_plannerflows.py
```

로컬 학습 시 참고:
- GPU가 없으면 자동으로 CPU 사용 (느리지만 가능)
- Mac의 경우 MPS backend 사용
- 학습 시간이 오래 걸릴 수 있으므로 epoch 수를 줄이는 것도 고려
...
// turbo
```bash
rsync -avP -e "ssh -p 22" phan@166.104.224.180:~/RLNF-RRT/result/visualization/loss /home/phan/Desktop/research/RLNF-RRT/result/
...
- Remote Dir: `~/RLNF-RRT`
- Dataset: 서버에 저장됨
- 결과: `result/visualization/loss/`, `result/models/`